...
public class JenaBasedVoidParsingSupplierDecorator extends AbstractDocumentSupplierDecorator {
    ...
    private void parseVoid(Document document, String text) {
        ...
        VoidParser vParser = new VoidParser();
        RDFParser parser = RDFParser.create().base(baseURI).fromString(text).lang(Lang.TTL).build();
        try {
            parser.parse(vParser);
            // streamer.runExtraction(text, baseURI, Lang.TTL, parser);
            vParser.addVoidToDocument(document);
        } catch (Exception e) {
            LOGGER.error("Got an exception when parsing the content of document " + document.toString() + ".", e);
        }
    }
    ...
    protected class VoidParser implements StreamRDF {
        ...
        public void triple(Triple triple) {
            try {
                ...
                if (object.equals(VOID.Dataset.asNode()) && (predicate.equals(RDF.type.asNode()))) {
                    ...
                    classes.put(subject.getBlankNodeLabel(),
                            object.isBlank() ? object.getBlankNodeLabel() : object.getURI());
                    ...
                    specialClasses.put(subject.getBlankNodeLabel(),
                            object.isBlank() ? object.getBlankNodeLabel() : object.getURI());
                    ...
                    properties.put(subject.getBlankNodeLabel(),
                            object.isBlank() ? object.getBlankNodeLabel() : object.getURI());
